...
export function ClipUploadPanel({
  ...
  async function uploadToTikTok() {
    ...
    setMessage(
      "TikTok upload queued. Start npm run worker:reap-publish to process it.",
    );
    setIsUploading(false);
    refresh();
  }
  ...
  return (
    <section className="grid gap-3 rounded-xl border border-[rgba(223,254,0,0.15)] bg-[#1e2130] p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-[#c6c9ab]">
            Queues the Reap publish worker with this clip file and current
            metadata.
          </p>
        </div>
        {latestTikTokUpload ? (
          <StatusBadge status={`tiktok ${latestTikTokUpload.uploadStatus}`} />
        ) : (
          <StatusBadge status="not queued" />
        )}
      </div>
        ...
        <div className="rounded-lg border border-[rgba(223,254,0,0.15)] bg-[rgba(22,21,20,0.84)] px-4 py-3 text-sm leading-6 text-[#c6c9ab]">
          <p>
            ...
            <span className="font-bold text-[#e2e2e1]">
              {formatDate(latestTikTokUpload.createdAt)}
            </span>
          </p>
          ...
          {latestTikTokUpload.errorMessage ? (
      ...
      {message ? (
        <p className="rounded-lg border border-[#dffe00] bg-[rgba(57,255,20,0.10)] px-4 py-3 text-sm font-bold text-[#dffe00]">
          {message}
        </p>
      ) : null}
      {error ? (
        <p className="rounded-lg border border-[#ffb4ab] bg-[rgba(255,180,171,0.10)] px-4 py-3 text-sm font-bold text-[#ffb4ab]">
          {error}
        </p>
      ) : null}
    </section>
