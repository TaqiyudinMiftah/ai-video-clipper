        ...
        <div className="overflow-hidden rounded-xl border border-[rgba(223,254,0,0.15)]">
          ...
          {videos.length ? (
              ...
              >
                <div>
                  <h3 className="font-[family-name:var(--font-display)] text-lg font-black tracking-[-0.04em] text-white">
                    {video.title || "Untitled video task"}
                  </h3>
                  <p className="text-sm text-[#c6c9ab]">
                    Created {formatDate(video.createdAt)}
                  </p>
                  {video.errorMessage ? (
                    <p className="mt-2 rounded-lg border border-[#ffb4ab] bg-[rgba(255,180,171,0.10)] px-3 py-2 text-sm font-bold text-[#ffb4ab]">
                      {video.errorMessage}
                    </p>
                  ) : null}
                </div>
                <p className="truncate text-sm text-[#c6c9ab]">
                  {video.sourceUrl ||
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <DeleteVideoButton id={video.id} compact />
                </div>
              </article>
            ))
          ) : (
            <div className="rounded-none border border-x-0 border-b-0 border-t border-[rgba(223,254,0,0.15)] bg-[rgba(30,32,32,0.70)] p-8 text-center">
              ...
              <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#c6c9ab]">
                Create a URL or file upload task and the queue will appear here
                with retryable failure states.
              </p>
            </div>
          )}
        </div>
