...
export default function NewVideoPage() {
  return (
    <AppShell
      eyebrow="New Task"
      title="Drop in a source video, then configure the cut."
      description="Submit a URL or upload an MP4, MOV, or WEBM source file. The next screen lets you choose Reap genre, captions, language, orientation, and clip length before queueing."
      ...
      <div className="grid gap-5 lg:grid-cols-12">
        ...
        <aside className="rounded-xl border border-[rgba(223,254,0,0.15)] bg-[rgba(22,21,20,0.84)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.40)] backdrop-blur-xl lg:col-span-4">
          ...
          <div className="mt-5 grid gap-4 text-sm leading-7 text-[#c6c9ab]">
            ...
            <p>Reap API handles clip generation and TikTok publishing through Reap Publish. You choose clipping settings before the worker starts.</p>
          </div>
          <div className="mt-6 grid gap-3">
            {["Create source draft", "Choose Reap config", "Enqueue BullMQ job", "Store clips server-side"].map((item) => (
