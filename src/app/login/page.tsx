      ...
      <section className="mx-auto grid min-h-screen w-full max-w-[1280px] content-center gap-8 px-5 py-24 md:px-10">
        <div className="max-w-3xl">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-[#c6c9ab]">
            Each operator gets isolated video tasks, clip records, upload
            targets, and logs. Production access uses OAuth; local development
            may still use the explicit dev auth fallback.
          </p>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-12">
          <section className="rounded-xl border border-[rgba(223,254,0,0.15)] bg-[rgba(22,21,20,0.84)] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.40)] backdrop-blur-xl lg:col-span-5">
            ...
            <p className="mt-6 text-sm leading-6 text-[#c6c9ab]">
              New here?{" "}
              <Link
                href={getSignupHref(callbackUrl)}
                className="font-bold text-[#dffe00] transition hover:text-[#39ff14]"
              >
                Create an account
              </Link>
              .
          ...
          <aside className="rounded-xl border border-[rgba(223,254,0,0.15)] bg-[rgba(22,21,20,0.84)] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.40)] backdrop-blur-xl lg:col-span-7">
            ...
            <div className="mt-5 grid gap-4 text-sm leading-7 text-[#c6c9ab]">
              <p>
                Videos, clips, uploads, jobs, and logs are always scoped to the
                signed-in user.
              </p>
              <p>
                Webhook processing remains server-side and uses Reap project
                IDs; browser users never receive Reap or storage service keys.
              </p>
              <p>
                For the MVP, Reap is still app-level. User-level Reap/TikTok
                integrations should be added after the core workflow is stable.
              </p>
            </div>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                href="/terms"
                className="inline-flex min-h-11 items-center justify-center rounded-lg border border-[rgba(223,254,0,0.15)] bg-[#1e2130] px-4 py-2.5 font-[family-name:var(--font-mono)] text-xs font-bold uppercase tracking-[0.16em] text-[#c6c9ab] transition hover:-translate-y-0.5 hover:border-[rgba(223,254,0,0.42)] hover:text-[#dffe00]"
              >
                Terms
              </Link>
              <Link
